...
def find_rocks(img, rgb_thresh = (110,110,50)):
    color_select = np.zeros_like(img[:,:,0])
    rocks_select =(img[:,:,0] > rgb_thresh[0]) \
                & (img[:,:,1] > rgb_thresh[1]) \
                & (img[:,:,2] < rgb_thresh[2])

    color_select[rocks_select] = 1
    return color_select
...
def perception_step(Rover):
    ...
    obs_map = np.abs(np.float32(threshed) - 1) * mask
    ...
    Rover.worldmap[y_pix_world, x_pix_world, 2] += 2
    ...
    if rocks_map.any():
        ...
        Rover.nav_dists, Rover.nav_angles = to_polar_coords(rock_x, rock_y)
        rock_idx = np.argmin(Rover.nav_dists)
        rock_xcen = rock_x_world[rock_idx]
        rock_ycen = rock_y_world[rock_idx]
        Rover.worldmap[rock_ycen,rock_xcen, 1] =255
        if len(rock_x_world) > 3:
            Rover.mode = 'rock'
